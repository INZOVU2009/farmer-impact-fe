...
const AssignedParchmentTable = () => {
  ...
  const inDryStorageParchments = assignedParchments?.filter(
    (parchment) => getTransactionState(parchment.cherry_lot_id) == "in-dry-storage"
  );
  ...
  const dryingParchmentA = parchmentA?.filter((parchA)=>getTransactionState(parchA.cherry_lot_id) == 'in-drying')
  const inDryStorageParchmentA = parchmentA?.filter((parchA)=>getTransactionState(parchA.cherry_lot_id) == 'in-dry-storage')
  const inTransitParchmentA = parchmentA?.filter((parchA) => getTransactionState(parchA.cherry_lot_id) == "in-transit");
  const inDeliveredParchmentA= parchmentA?.filter((parchA) => getTransactionState(parchA.cherry_lot_id) == "delivered"
  );
  const dryingParchmentB = parchmentB?.filter((parchB)=>getTransactionState(parchB.cherry_lot_id) == 'in-drying')
  const inDryStorageParchmentB= parchmentB?.filter((parchB)=>getTransactionState(parchB.cherry_lot_id) == 'in-dry-storage')
  const inTransitParchmentB= parchmentB?.filter((parchB) => getTransactionState(parchB.cherry_lot_id) == "in-transit");
  const inDeliveredParchmentB= parchmentB?.filter((parchB) => getTransactionState(parchB.cherry_lot_id) == "delivered");
  const dryingParchmentC = parchmentC?.filter((parchC)=>getTransactionState(parchC.cherry_lot_id) == 'in-drying')
  const inDryStorageParchmentC = parchmentC?.filter((parchC)=>getTransactionState(parchC.cherry_lot_id) == 'in-dry-storage')
  const inTransitParchmentC= parchmentC?.filter((parchC) => getTransactionState(parchC.cherry_lot_id) == "in-transit");
  const inDeliveredParchmentC= parchmentC?.filter((parchC) => getTransactionState(parchC.cherry_lot_id) == "delivered");

  const totalWeightDrying = dryingParchments?.reduce((total, parchment) => total + parseInt(parchment.parch_weight),0);
  const totalWeightInDryStorage= inDryStorageParchments?.reduce((total, parchment) => total + parseInt(parchment.parch_weight),0);
  const totalWeightInTransit= inTransitParchments?.reduce((total, parchment) => total + parseInt(parchment.parch_weight),0);
  const totalWeightInDelivered= inDeliveredParchments?.reduce((total, parchment) => total + parseInt(parchment.parch_weight),0);

  const totalDryingA = dryingParchmentA?.reduce( (total,parchA)=> total + parseInt(parchA.parch_weight),0)
  const totalDryStorageA = inDryStorageParchmentA?.reduce( (total,parchA)=> total + parseInt(parchA.parch_weight),0)
  const totalTransitA = inTransitParchmentA?.reduce( (total,parchA)=> total + parseInt(parchA.parch_weight),0)
  const totalDeliveredA = inDeliveredParchmentA?.reduce( (total,parchA)=> total + parseInt(parchA.parch_weight),0)

const totalDryingB = dryingParchmentB?.reduce((total,parchB)=> total + parseInt(parchB.parch_weight),0)
const totalDryStorageB= inDryStorageParchmentB?.reduce( (total,parchB)=> total + parseInt(parchB.parch_weight),0)
const totalTransitB = inTransitParchmentB?.reduce( (total,parchB)=> total + parseInt(parchB.parch_weight),0)
const totalDeliveredB= inDeliveredParchmentB?.reduce( (total,parchB)=> total + parseInt(parchB.parch_weight),0)

const totalDryingC = dryingParchmentC?.reduce((total,parchC)=> total + parseInt(parchC.parch_weight),0)
const totalDryStorageC = inDryStorageParchmentC?.reduce( (total,parchC)=> total + parseInt(parchC.parch_weight),0)
const totalTransitC = inTransitParchmentC?.reduce( (total,parchC)=> total + parseInt(parchC.parch_weight),0)
const totalDeliveredC = inDeliveredParchmentC?.reduce( (total,parchC)=> total + parseInt(parchC.parch_weight),0)
              ...
              <table className="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                ...
                {assignedParchments?.map((parchment, index) => (
                  ...
                  {formatDate(parchment.created_at)}
                    ...
                    <td className="p-4 text-base font-medium text-gray-500 whitespace-nowrap dark:text-white">
                  {parchment.parch_grade.charAt(parchment.parch_grade.length - 1)}

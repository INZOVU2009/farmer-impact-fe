...
const DigitalLoadingFormTable = ({parchments, stationName, StationID, decodedToken, stockbal, handleSelectChange, selectedParchment, selectedOption, handleSelectedOption, formData, handleFormData,handleSubmit, handleClick, Spinner,isloading}) => {
...
{/* <select
    ...
    ? stockbal(parchment)// If the parchment is selected, calculate stock balance
    : parchment.parch_weight // Otherwise, show the original parchment weight
  } */} {stockbal(parchment)}
            </td>
            <td 
            className="p-4 text-base font-medium text-gray-500 whitespace-nowrap dark:text-white"
...
{selectedParchment === parchment.parchment_id  && (
 ...
 { selectedParchment === parchment.parchment_id &&selectedOption &&(
     <input
       type="text"
       name={`bags_of_parchment_left`} // Use a unique name for each input field
       className="rounded-lg w-40"
